...
function simulate_data_gl(n,grpsize,m,SNR,sigma,k)
    ...
    shuffle!(indic)
    for i = 1:m
         thetas[i] = indic[i]*randn(grpsize,1)
...
function loss_test(Zall, y, hbeta, grpsizes, indxs, n,Rs,p)   #print(hbeta[indxs[j]])
    hbetan = group_beta(hbeta,Rs,m,grpsizes,indxs,p);
    return ((0.5*norm(y - Zall*hbetan)^2))/n
end

function group_beta(theta,Rs,m,grpsizes,indxs,p)
    beta = zeros(p,1)
    for j = 1:m
        beta[indxs[j]] = Rs[j]\theta[indxs[j]] 
    end
    return beta
end

function center_columns_matrix_array(Z_array::Vector{Any})
...
function loss(Z,y,hbeta, grpsize,m,n, lambda)
   Zall = reduce(hcat, Z);
   return ((0.5*norm(y- Zall*hbeta)^2) + sqrt(grpsize)*lambda*sum([norm(hbeta[(j-1)*grpsize+1:j*grpsize]) for j = 1:m]))/n
end
